...
4. **Pick the voice（Owner 指令 2026-09-16，默认必问）** — 动笔前向
   ...
   - **changelog 风格（首选）**：客观且直观、通俗易懂且准确，无晦涩
     ...
     词汇（tone-laws 第 7 条的词汇源纪律照用）——**内部实现层的自造
     词与架构隐喻（如「双纪元」「隔离的 home」一类）禁止进标题与
     摘要**；标题写用户能认出的事实名词（功能名、修复对象、数字），
     克制、专业、新闻风格（Owner 指令 2026-09-16 复审）。
...
8. **Owner 审稿门（Owner 指令 2026-09-16，硬门）** — 全部门禁绿后，
   **提交 Owner 审稿；未获明确批准前禁止 commit/push/部署**。审稿
   物料：标题与摘要、双语全文（文件路径或贴文）、配图清单、门禁
   输出。Owner 提出修改 → 回到第 5 步改稿重走门禁再审；批准 →
   才进入 commit/push/deploy。**没有任何「隐式批准」**：Owner 此前
   授权过发版、验收过产品、或指定过风格，都不构成对本篇文章的
   放行（2026-09-16 实证：产品验收通过 ≠ 发版文免审）。
...
8. 标题/摘要用内部自造词或架构隐喻代替用户可辨认的事实（「双纪元」
   「隔离的 home」是 2026-09-16 实证打回例）
9. 未经 Owner 审稿批准就 commit/push/部署（2026-09-16 实证打回例；
   见 Workflow 第 8 步硬门）
...
- [ ] **Owner 审稿已获明确批准**（门禁全绿 ≠ 放行；见 Workflow 第 8 步。
      未批准时工作止于本地，不 commit 不 push 不部署）
...
- [ ] 配图是**展示区域的裁剪图**（不是整页视口），每张单看就能知道它
  证明了什么；两稿图清单一致，且「文字是主体」的图语言与稿件配套
  （判定的问句与分层检查见 references/screenshots.md 第 9 条）；换图后
  重建过站点
