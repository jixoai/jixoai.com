...
9. **截图语言必须与稿件语言配套**（Owner 法令 2026-09-16，OpenTray
   v0.28 博客实证）：图里含界面文案（向导开关、菜单、提示语）的截图，
   中文稿用产品中文界面的截图，英文稿用英文界面的截图——英文文里放
   中文截图会被退回。只有图内**不含可读文案**（纯图形、代码块、图表）
   才允许中英共用同一张。落位按语言分组：`<slug>/wizard-toggle.zh.png`
   与 `<slug>/wizard-toggle.en.png`（或 `zh/`、`en/` 子目录），文件名即
   语言标记。交稿前用 `grep -o` 对比两稿图清单，逐张核对语言标记与稿件
   语言一致——不是简单「两稿同一组」。目标产品有本地化界面时（如
   create-opentray 向导的九语言，`?lang=` / `x-opentray-locale` 切换），
   切语言重截同一区块即可；没有本地化的界面不要截文案进图，或裁剪时
   避开文案区域。
