...
9. **区分「文字是辅助」与「文字是主体」**（Owner 法令 2026-09-16，
   OpenTray v0.28 博客实证）。默认维持第 8 条「中英两稿引用同一组图」：
   大部分截图是效果图——证明的是界面形态、布局、组件、配色，UI 文字
   只是画面元素，外语界面出现在中文文章里（或反之）完全可读。这类图
   不需要按语言重截。
   **例外**：当图要证明的内容**就是那段文字本身**——本地化结果、开关
   标签、菜单项、提示语，读者必须读懂图里的字才能获得文章的主张——
   截图语言必须与稿件语言配套：中文稿用中文界面，英文稿用英文界面，
   分开截、分开落位（`<slug>/wizard-toggle.zh.png` 与 `.en.png`，或
   `zh/`、`en/` 子目录），文件名即语言标记。判定问句：「这段字看不懂，
   读者还能从图里得到文章主张的东西吗？」能 → 共用同一张；不能 →
   按语言分截。目标产品有本地化界面时（如 create-opentray 向导的九种
   语言，`?lang=` / `x-opentray-locale` 切换），切语言重截同一区块即可。
   交稿检查相应分层：辅助文字的图核对两稿清单一致；主体文字的图逐张
   核对语言标记与稿件语言一致。
