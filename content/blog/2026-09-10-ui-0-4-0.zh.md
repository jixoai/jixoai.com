---
title: "Jixoai UI v0.4.0：特效改为附件，registry 净增 23 条"
date: 2026-09-10T18:49:06Z
description: jixoai-ui v0.4.0 的按压特效改为 Svelte 附件（{@attach pressEffect(...)}），同时落地六引擎高亮、图标管线、prose 排版、KaTeX 与 Mermaid、Card 结构内核与浮动画布 dock。registry 从 113 条涨到 136 条。三处迁移动作都不留兼容层。
author: jixoai
tags: [ui]
repo: ui
version: "0.4.0"
...
![rainbow 画布：三个中性宿主按钮 upgrade / subscribe / ⌘ 共用同一个特效对象，宿主自己的边框就是流动的彩虹环，环宽由 ringW 驱动，填充走 auto / null / solidFill 三态](/blog-assets/2026-09-10-ui-0-4-0/effects-dock.png)

特效从此有了自己的主页 `/docs/effects.html`，玻璃的双层游乐场整体搬了进去，原路由 `/docs/components/glass.html` 退场；命令行认分组别名 `npx jixoai-ui add effects`。

![effects 主页：家族法则，特效是挂在元素上的附件工厂而不是组件；五枚小签列出两种挂载通道、动作桥与挂载点属性](/blog-assets/2026-09-10-ui-0-4-0/effects-home.png)
...
![code-card 的完整结构：文件名签 spawn.ts、右上 shiki · ts 引擎徽章、脚注 powered by Shiki · theme: jixoai 与 copy 控件](/blog-assets/2026-09-10-ui-0-4-0/highlight-engine.png)
...
![icons 页的组件面：Icon 组件的 size 梯级，同一个 prop 在 12 / 16 / 24 / 32 四档上实渲染](/blog-assets/2026-09-10-ui-0-4-0/icons.png)
...
![prose 页的旋钮展示：左栏 size 与 leading 走继承缩放（ambient 14px 对 17px），右栏 align justify 加 hyphens 与 wrap pretty](/blog-assets/2026-09-10-ui-0-4-0/prose.png)
...
![card 页：结构内核与 head / body / foot 三段带，foot 里是行内动作座 Cancel / Save](/blog-assets/2026-09-10-ui-0-4-0/card.png)
...
![component-canvas：画布套画布，外层 LIVE 舞台内嵌一个简化实例，两层各有自己的 Code 栏](/blog-assets/2026-09-10-ui-0-4-0/canvas.png)
...
- 升级：见上面的迁移节（改挂载方式、按新名字重新加条目，玻璃走 `blur()` 的默认值）
