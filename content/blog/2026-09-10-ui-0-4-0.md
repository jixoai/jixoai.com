---
title: "Jixoai UI v0.4.0: effects become attachments, plus 23 new registry items"
date: 2026-09-10T18:49:06Z
description: jixoai-ui v0.4.0 turns the press effects into Svelte attachments ({@attach pressEffect(...)}) and lands six-engine highlighting, the icon pipeline, the prose scope, KaTeX and Mermaid, the Card structural kernel and a floating canvas dock. The registry goes from 113 items to 136, with two breaking changes and no compat layer.
author: jixoai
tags: [ui]
repo: ui
version: "0.4.0"
...
Jixoai UI v0.4.0 is out (2026-09-10). 175 commits, 41 of them feat, 49 fix and 66 docs, with the registry up from 113 items to 136. The headline is effects becoming attachments; the same window also lands highlighting, icons, typography, math and diagrams, surface structure, and the canvas. Two breaking changes, neither with a compat layer.
...
![The rainbow playground: ringW drives the ring width, and the fill switches between auto, transparent and a solid colour](/blog-assets/2026-09-10-ui-0-4-0/effects-dock.png)
...
![The effects home: the family law, the two mount channels, and npx jixoai-ui add effects](/blog-assets/2026-09-10-ui-0-4-0/effects-home.png)
...
![code-card and its dock: ENGINE switches between the six backends, with LANG and THEME independent](/blog-assets/2026-09-10-ui-0-4-0/highlight-engine.png)

## Icons: one pipeline, two faces, one component

The icon system gained its component face: `<Icon name="check" />` with a type-safe `IconName` union and `size` and `strokeWidth` props, replacing 197 `{@html icons.x}` injection sites.
...
![The icons page: one pipeline, two faces, one component](/blog-assets/2026-09-10-ui-0-4-0/icons.png)
...
The reading-content family extracts every markdown construct that had no registry part. `text.svelte` implements `<Text mark="…">` and exports `P Strong Em Del Mark Ins Sub Sup` from the same module; blockquote carries both an inset shadow and a `border-inline-start` variant at 1px, 4px and 8px; Link marks external links with a suffix icon by default; List splits into the strict ol and ul cases. The markdown face itself is streaming, mapping its AST onto registry parts, with code blocks riding code-card's generation-guard async repaint.

![The prose page: eleven knobs and two channels](/blog-assets/2026-09-10-ui-0-4-0/prose.png)
...
![The card page: the structural kernel and the three bands](/blog-assets/2026-09-10-ui-0-4-0/card.png)
...
The companion canvas-everywhere sweep routes every docs demo through the canvas. Previously 443 ComponentCanvas instances stood against 1580 SectionCard instances, leaving readers unable to see the code behind most of what they saw; demos now carry a source lane by default.

![component-canvas: the documentation workbench and its floating dock](/blog-assets/2026-09-10-ui-0-4-0/canvas.png)
...
`@jixoai/ui-vite-plugin` 0.3.0 ships alongside: typography brings the prose scope and the parts' R2/R3, icons brings the real-project E2E tier above, and canvas brings the dock's acceptance rulings (the acrylic popover material, the grip-vertical icon, and `position: absolute` retired). The suite is at 398/398.

## Migration (breaking changes)

Two breaking changes, neither with a compat layer.

The first is how effects mount, together with two item renames:
...
`alert-dialog` becomes `system-dialog`, and `icons` splits into `icon` and `icon-set`. The lowest-friction path is to re-add items under the new names:

```bash
npx jixoai-ui add system-dialog icon icon-set
```

The second is glass: the hand-tuned `.jx-glass` class retires and `blur()`'s defaults take over (14px blur, 1.35 saturation, 68% fill), so an unconfigured migration is computed-equivalent. The full mounting map:
...
- Upgrade: see the Migration section above (change the mounting form, re-add items under the new names)
